...
export default function Chapter9Screen() {
    ...
    const scenes: Scene[] = t.chapter9.storySegments.map((segment, index) => ({
        text: segment.text,
    ...
    const handleFinish = () => {
        if (isLastScene && !isAnimating.value) {
            console.log('Story Finished - Return to Home');
            // Retour à la racine de l'application (l'écran d'accueil)
            router.dismissAll();
            router.replace('/');
        }
    };
